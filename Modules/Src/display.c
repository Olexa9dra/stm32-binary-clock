...
void Display_Show(uint16_t value, DisplayMode mode) {
  ...
  /*
   * Error blink has priority over normal selected-column blink.
   */
  ...
  for (uint8_t column = 0; column < DISPLAY_COLUMN_COUNT; column++) {
    ...
    /*
     * Normal selected-column blink.
     *
     * This is completely disabled while errorColumns is active.
     */
...
void Display_SetBlinkColumn(DisplayColumnMask column) {
  ...
  /*
   * Start normal blinking from visible state.
   */
  blinkVisible = 1;
